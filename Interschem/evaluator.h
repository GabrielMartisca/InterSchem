...
string infixToPostfix(string infix)
{
    ...
    for (int i = 0; i < infix.length(); i++)
    {
        ...
        else if (isOperator(infix[i]))
        {
            ...
            else if (priority(temp) > priority(s.top()))
            {
                s.push(temp);
            }
            else if (priority(temp) == priority(s.top()) && temp == "^")
            {
                s.push(temp);
            }
            else
            {
                while (!s.empty() && priority(temp) < priority(s.top()))
                {
                    string temp = s.top();
                    postfix += " " + temp;
                    s.pop();
...
void evaluateOperation(string s)
{
    cout << EvaluatePostfix(convertToVec(infixToPostfix(s))) << '\n';
}